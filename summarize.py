...
# Prompt used when providing the whole paper as a PDF attachment
USER_PROMPT_PDF = """Answer the following questions about the attached paper in a podcast-friendly tone:
1. What is the title of the paper?
2. What institution(s) the authors come from?
3. What is the problem addressed by the paper?
4. Why is it an interesting problem?
...
Use the provided PDF to answer these questions. Don't start with an intro, dive straight into the paper. Keep it short and concise."""
...
def create_summary_from_pdf(pdf_path):
    """Create a summary of a paper by uploading the entire PDF to OpenAI."""
    logger.info(f"Creating paper summary from PDF {pdf_path}")
    ...
    try:
        ...
        logger.error(f"Error creating summary from PDF: {str(e)}")
        raise
